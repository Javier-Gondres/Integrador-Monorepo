...
function formatPerformedBy(row: WasteListItem): string {
  if (!row.performedBy?.user) return "—";
  return `${row.performedBy.user.firstName} ${row.performedBy.user.lastName}`.trim();
}
...
export function WasteTableContainer({
  ...
  const columns: DataTableColumn<WasteListItem>[] = [
    ...
    },
    {
      ...
      align: "left",
      cell: (row) => (
        <span className="text-gray-700">{formatPerformedBy(row)}</span>
      ),
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      <DataTableToolbar
        ...
        onCreate={onCreate}
        createPermission={Permission.INVENTORY_ADJUST}
      />
