...
function getDaysRemaining(dueDateStr: string): number {
  return differenceInCalendarDays(parseISO(dueDateStr), startOfToday());
}
      ...
      cell: (item) => (
        ...
        >
          {item.purchase.invoiceNumber ||
            `C-${item.purchase.id.slice(-6).toUpperCase()}`}
        </span>
      ),
    },
    {
      ...
      header: "Proveedor",
      cell: (item) => (
        <span className="text-body font-medium">{item.supplier.name}</span>
      ),
    },
    {
      id: "createdAt",
      header: "Fecha",
      cell: (item) => (
        <span className="text-body">{formatDate(item.createdAt)}</span>
      ),
    },
    {
      id: "dueDate",
