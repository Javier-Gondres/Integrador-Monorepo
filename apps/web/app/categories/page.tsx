...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      try {
        ...
        const payload = unwrapApiData<PaginatedResponse>(json);
  ...
  const cambiarEstado = async (id: string, cur: boolean) => {
    ...
    try {
      const res = await fetch(`${API_URL}/categories/${id}/${action}`, {
        method: "PATCH",
        headers: getApiHeaders(),
      });
      if (res.ok) {
        setData((prev) =>
          prev.map((c) => (c.id === id ? { ...c, isActive: !cur } : c)),
        );
      }
    } catch (e) {
      console.error(e);
                  ...
                  data.map((cat, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "14px 20px", textAlign: "center" }}>
                        <button
                          onClick={() => cambiarEstado(cat.id, cat.isActive)}
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: "5px",
                            ...
                            fontWeight: 500,
                            cursor: "pointer",
                            border: `1px solid ${cat.isActive ? C.greenBorder : C.grayBorder}`,
                            backgroundColor: cat.isActive ? C.greenBg : C.grayBg,
                            color: cat.isActive ? C.greenText : C.grayText,
                          }}
                          title={
                            cat.isActive
