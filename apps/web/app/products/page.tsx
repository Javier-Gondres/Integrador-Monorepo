...
import { API_URL, getApiHeaders, unwrapApiData } from "../../lib/api-client";
  ...
  const fetchCategories = useCallback(
    async (pageNum: number, isLoadMore = false) => {
      ...
      try {
        ...
        const payload = unwrapApiData<PaginatedResponse<Category>>(json);
        setTotalPages(payload.meta.totalPages);

        if (isLoadMore) {
          setCategories((prev) => [...prev, ...payload.items]);
        } else {
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      try {
        ...
        const json = await res.json();
        const payload = unwrapApiData<PaginatedResponse<Product>>(json);
        setData(payload.items);
        setTotal(payload.meta.total);
        setTotalPages(payload.meta.totalPages);

        if (currentPage > payload.meta.totalPages) {
          setCurrentPage(payload.meta.totalPages || 1);
        }
      } catch (e) {
  ...
  const cambiarEstado = async (id: string, cur: boolean) => {
    const action = cur ? "deactivate" : "activate";
    try {
      const res = await fetch(`${API_URL}/products/${id}/${action}`, {
        method: "PATCH",
        headers: getApiHeaders(),
      });
      if (res.ok) {
        setData((prev) =>
          prev.map((p) => (p.id === id ? { ...p, isActive: !cur } : p))
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const guardarProducto = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ name: "", code: "", description: "", price: 0, isActive: true });
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: C.tableHead }}>
                  {["Código", "Nombre", "Descripción", "Precio", "Estado", "Acciones"].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: "13px 20px",
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={6} style={{ textAlign: "center", padding: "48px", color: C.mutedText, fontSize: "14px" }}>
                      Cargando productos...
                    </td>
                  </tr>
                ) : data.length === 0 ? (
                  ...
                  data.map((product, i) => (
                    ...
                    >
                      <td style={{ padding: "14px 20px", fontSize: "13px", color: C.headText, textAlign: "center" }}>
                        {product.code}
                      </td>
                      ...
                      <td style={{ padding: "14px 20px", textAlign: "center" }}>
                        <button
                          onClick={() => cambiarEstado(product.id, product.isActive)}
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: "5px",
                            ...
                            fontWeight: 500,
                            cursor: "pointer",
                            border: `1px solid ${product.isActive ? C.greenBorder : C.grayBorder}`,
                            backgroundColor: product.isActive ? C.greenBg : C.grayBg,
                            color: product.isActive ? C.greenText : C.grayText,
                          }}
                          title={product.isActive ? "Click para desactivar" : "Click para activar"}
                        >
                          {product.isActive ? (
                            <>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                <label style={{ fontSize: "13px", fontWeight: 600, color: C.bodyText }}>
                  Código (SKU) <span style={{ color: C.danger }}>*</span>
                </label>
                <input
                  type="text"
                  required
