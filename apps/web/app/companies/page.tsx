...
export default function CompaniesPage() {
  ...
  const cargarCompanies = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      alert("No se pudieron cargar las empresas. Intenta de nuevo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // Borrado lógico
  const borradoLogico = async (slug: string) => {
    try {
      ...
      setCompanies((prevCompanies) =>
        prevCompanies.map((c) =>
          c.slug === slug ? { ...c, isActive: false } : c,
        ),
      );
    } catch (error: unknown) {
  ...
  const guardarCompany = async (e: React.FormEvent) => {
    ...
    const url = editingCompany
      ? `${API_URL}/companies/${editingCompany.slug}`
      : `${API_URL}/companies`;
                    ...
                    >
                      ...
                      <td style={{ padding: "14px 20px", textAlign: "center" }}>
                        ...
                        >
                          ...
                          <button
                            onClick={() => borradoLogico(company.slug)}
                            title="Eliminar"
                            style={{
                              width: "34px",
                              height: "34px",
