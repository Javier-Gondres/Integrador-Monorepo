...
export default function BranchesPage() {
  ...
  const cargarBranches = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      alert("No se pudieron cargar las sucursales. Intenta de nuevo.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const cargarCompanyName = async (signal?: AbortSignal) => {
    try {
      const res = await fetch(`${API_URL}/companies/${companySlug}`, {
        cache: "no-store",
        signal,
        headers: {
          Accept: "application/json",
  ...
  const borradoLogico = async (id: string) => {
    try {
      const res = await fetch(
        `${API_URL}/companies/${companySlug}/branch/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const guardarBranch = async (e: React.FormEvent) => {
    ...
    const url = editingBranch
      ? `${API_URL}/companies/${companySlug}/branch/${editingBranch.id}`
      : `${API_URL}/companies/${companySlug}/branch`;
