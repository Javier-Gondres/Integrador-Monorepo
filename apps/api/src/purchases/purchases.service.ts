...
import { ProductSupplierRepository } from 'src/product-supplier/product-supplier.repository';
  ...
  async create(dto: CreatePurchaseDto, company: CompanyContext) {
    ...
    const items = await Promise.all(
      dto.items.map(async (item) => {
        const catalogLink = await this.productSupplierRepository.findOne(
          dto.supplierId,
          item.productId,
        );

        if (!catalogLink?.isActive) {
          throw new BusinessException(
            ErrorCodes.VALIDATION_ERROR,
            'El producto no está activo en el catálogo del proveedor',
          );
        }

        if (catalogLink.lastCost === null) {
          throw new BusinessException(
            ErrorCodes.VALIDATION_ERROR,
            'El producto no tiene costo registrado en el catálogo del proveedor',
          );
        }

        const unitCost = Number(catalogLink.lastCost);

        return {
          productId: item.productId,
          quantity: item.quantity,
          unitCost,
          subtotal: round2(item.quantity * unitCost),
        };
      }),
    );
